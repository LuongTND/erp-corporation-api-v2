...
public sealed class GetMyStoreMembersQueryHandler(IUnitOfWork unitOfWork, IUserContext userContext)
    : IRequestHandler<GetMyStoreMembersQuery, IEnumerable<StoreMemberResponse>>
{
    public async Task<IEnumerable<StoreMemberResponse>> Handle(GetMyStoreMembersQuery _, CancellationToken ct)
    {
        var store = await unitOfWork.Repository<Store>()
            .FindAsync(s => s.ManagerId == userContext.UserId && !s.IsDeleted, ct);
        if (store is null) return [];

        // ponytail: inline instead of mediator dispatch — avoids extra allocation and handler resolution
        var memberships = (await unitOfWork.Repository<UserStore>().GetPagedAsync(
            new QueryInfo { Top = 10000, NeedTotalCount = false },
            filter: us => us.StoreId == store.Id && us.IsActive,
            ct: ct)).Items.ToList();
